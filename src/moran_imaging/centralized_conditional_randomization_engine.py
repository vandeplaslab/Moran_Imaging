...
def crand(
    ...
    Parameters
    ----------
    z : ndarray                                # 1D array in my case!
        2D array with N rows with standardised observed values
    W : libpysal.weights.W                     # replace by W to be consistent
        Spatial weights object                 # nope, not in my case
    observed : ndarray
    ...
    """
    adj_matrix = W.sparse  # change

    n = len(z)
    if z.ndim == 1:
        ratio = (W.n - len(W.islands)) / W.s0
        scaling = ratio / (z * z).sum()
    else:
    ...
    with warnings.catch_warnings():
        # massive changes to sparsity incur a cost, but it's not large for simply changing the diag
...
def parallel_crand(
    ...
    # Warning: set n_jobs to the number of cores of your computer
    with parallel_backend("loky", n_jobs=52, inner_max_num_threads=1):
...
# @_njit(fastmath=True)
@njit(fastmath=False)
def moran_local_crand(i, z, permuted_ids, weights_i, scaling):
    self_weight = weights_i[0]
    other_weights = weights_i[1:]
    zi, zrand = prepare_univariate(i, z, permuted_ids, other_weights)
